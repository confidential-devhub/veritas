...
# Path: td-shim-tools/src/bin/td-shim-tee-info-hash/td_shim_tee_info_hash.py
#
# Python implementation of td-shim-tee-info-hash tool.
# This tool calculates MRTD (Measurement Register TD) value from the OVMF/TDVF binary image.
...
def parse_guid_from_bytes(guid_bytes: bytes) -> tuple:
    ...
    f0 = struct.unpack("<I", guid_bytes[0:4])[0]
    f1 = struct.unpack("<H", guid_bytes[4:6])[0]
    f2 = struct.unpack("<H", guid_bytes[6:8])[0]
    f3 = guid_bytes[8:16]
    return (f0, f1, f2, f3)
...
def guid_to_bytes(f0: int, f1: int, f2: int, f3: bytes) -> bytes:
    result = bytearray()
    result.extend(struct.pack("<I", f0))
    result.extend(struct.pack("<H", f1))
    result.extend(struct.pack("<H", f2))
    result.extend(f3)
    return bytes(result)
...
class TdxMetadataDescriptor:
    ...
    def __init__(self, signature, length, version, number_of_section_entry):
...
class TdxMetadataSection:
    ...
    def __init__(self, data_offset, raw_data_size, memory_address,
                 memory_data_size, section_type, attributes):
...
def compute_mrtd(image_path: str) -> str:
    """Compute MRTD from an OVMF/TDVF binary. Returns hex string."""
    with open(image_path, 'rb') as f:
        image_size = os.path.getsize(image_path)
        return build_mrtd(f, image_size).hex()
